...
<body>
	<?php
		if(!isset($_POST['dang_nhap'])){
			header("location:form_sign_in_admin.php?error_sign_in");
		}
		else{
			$email=$_POST['email'];
			...
			if($count==0){
				header("location:form_sign_in_admin.php?empty_data");
			}
			
			else{

				if(isset($_POST['cookie'])){
					setcookie('$row[\'ma_admin\']',$row['ma_admin'],time()+86400*60);
				}
