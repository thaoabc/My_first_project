...
<div class="container">
    ...
    ?>
      <form action="process_sign_up.php" method="post" class="form form-login">

        <span id="loi_ho_ten" style="color: lightblue"></span>
        <div class="form-field">
          <input type="text" id="ho_ten" name="ten_dang_ky" placeholder="Họ tên" autofocus required/>
        </div>
        ...
        <div class="form-field">
         ...
         <span id="loi_ngay_sinh" style="color: lightblue"></span>
         <span id="loi_thang_sinh" style="color: lightblue"></span>
         <span id="loi_nam_sinh" style="color: lightblue"></span>
        <div class="form-field">
          <p>Ngày sinh:</p>&emsp;
          <select name="Day" id="Tag" value="" required>
            <option name="Day" value="" disabled selected hidden>Ngày sinh</option>
          </select> &emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
          <select name="Month" id="Monat" value="">
            <option name="Month" value="" disabled selected hidden>Tháng sinh</option>
          </select> &emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
          <select name="Year" id="Jahrgang" value="">
            <option name="Year" value="" disabled selected hidden>Năm sinh</option>
          </select>
        </div>
        ...
        <script type="text/javascript">
          function yearOf() {
            var yearsList="";
            for(i= 2000; i<2019; i++) {
              yearsList = yearsList + "<option name=\"Year\" value=\""+i+"\""+">"+i+"</option>"
            }
            return yearsList;  
          }
          ...
          function displayYears() {
            var x = yearOf();
            document.getElementById("Jahrgang").innerHTML = "<option value=\"\" disabled selected hidden>Năm sinh</option>"+x;  
          }
          ...
          function monthOf() {
            var monthList="";
            for(i= 1; i<13; i++) {
              monthList = monthList + "<option name=\"Month\" value=\""+i+"\""+">"+i+"</option>"
            }
            return monthList;  
          }
          ...
          function displayMonths() {
            var x = monthOf();
            document.getElementById("Monat").innerHTML = "<option name=\"Month\"value=\"\" disabled selected hidden>Tháng sinh</option>"+x;  
          }
          ...
          function dayOf() {
            var dayList="";
            for(i=1; i<32; i++) {
              dayList = dayList + "<option name=\"Day\" value=\""+i+"\""+">"+i+"</option>"
            }
            return dayList;  
          }
          ...
          function displayDays() {
            var x = dayOf();
            document.getElementById("Tag").innerHTML = "<option name=\"Day\"value=\"\" disabled selected hidden>Ngày sinh</option>"+x;  
          }
...
<script type="text/javascript">
  function dang_ky(){
    ...
    var regex_ho_ten= /^[a-z\sàáâãèéêếìíòóôõùúăđĩũơưăạảấầẩẫậắằẳẵặẹẻẽềềểễệỉịọỏốồổỗộớờởỡợụủứừửữựỳỵỷỹ]+$/i;
    var result_ho_ten= regex_ho_ten.test(ho_ten);
    var regex_ho_ten_dung= /[a-z\sàáâãèéêếìíòóôõùúăđĩũơưăạảấầẩẫậắằẳẵặẹẻẽềềểễệỉịọỏốồổỗộớờởỡợụủứừửữựỳỵỷỹ]+/gi;
    ...
    if(sdt==""){
      ...
      var regex_dia_chi=/^[a-z\sàáâãèéêếìíòóôõùúăđĩũơưăạảấầẩẫậắằẳẵặẹẻẽềềểễệỉịọỏốồổỗộớờởỡợụủứừửữựỳỵỷỹ\.\,\0-9]{10,}$/i;
      ...
      var ngay_sinh = document.getElementById('Tag').value;
      if(ngay_sinh==0){
        document.getElementById('loi_ngay_sinh').innerHTML="Chưa nhập ngày sinh";
        document.getElementById('Tag').focus();
        return false;
      }
       else{
        document.getElementById('loi_ngay_sinh').style.display='none';
      }

      var thang_sinh = document.getElementById('Monat').value;
      if(thang_sinh==0){
        document.getElementById('loi_thang_sinh').innerHTML="Chưa nhập tháng sinh";
        document.getElementById('Monat').focus();
        return false;
      }
       else{
        document.getElementById('loi_thang_sinh').style.display='none';
      }

      var nam_sinh = document.getElementById('Jahrgang').value;
      if(nam_sinh==0){
        document.getElementById('loi_nam_sinh').innerHTML="Chưa nhập năm sinh";
        document.getElementById('Jahrgang').focus();
        return false;
      }
      else{
        document.getElementById('loi_nam_sinh').style.display='none';
      }
